...
resource "aws_api_gateway_integration" "connect_admin_post" {
  ...
  timeout_milliseconds    = "29000"
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:ap-southeast-1:lambda:path/2015-03-31/functions/arn:aws:lambda:ap-southeast-1:${var.environment.account_id}:function:${aws_lambda_function.lambda_outages.function_name}/invocations"
}
...
resource "aws_lambda_permission" "connect_admin" {
  ...
  function_name = "${aws_lambda_function.lambda_outages.function_name}"
